...
- **Make available to the E2E environment**: `validate-tt` MUST pull the CNI artifact by digest from staging (OIDC) onto the runner, then distribute the exact bytes to each centraluseuap **worker** node through multiple bounded `az vm run-command invoke` payloads (the only node-access path; §2 of the test doc). Chunks MUST carry ordering/offset metadata, be idempotent under transport retry, reassemble in a private node directory, and pass expected size + SHA-256 verification before install. `scripts/e2e/install-cni-tt.sh` then backs up the existing CNI to `/opt/cni/tt-backup-<ts>`, installs the binary, swaps the conflist, asserts `"mode": "transparent-tunnel"`, and restarts kubelet. The **control-plane node MUST remain on stock CNI**. No CNI hosting URL with an embedded credential/SAS token is committed (SEC-006).
...
- **Artifact-set release without rebuild**: the release job promotes **both** validated digests — the controller image (`az acr import`) and CNI OCI artifact (`oras copy`) — under the same `${SEMVER}`, verifies both and any moving-tag operation, then publishes a signed/attested `pod-nsg-release-index:${SEMVER}` OCI artifact recording the version and both exact repository digests. The index tag is the only atomic public commit point: consumers/audits MUST NOT treat either direct tag alone as a completed release (FR-024, RD-013/RD-014/RD-021). Before index commit, failure removes all prepared tags and restores moving tags.
...
- **FR-007**: The pipeline MUST tear down all `ss` and `xs` resources, including resource groups in both subscriptions, cross-region/cross-subscription role assignments, staging candidate tags, every per-run ACR pull token, NSG enforcement rules, transparent-tunnel pods, and cluster `imagePullSecret`s on success, failure, and cancellation, and MUST verify deletion in each owning subscription/registry. Candidate tags/tokens are deleted only by a final always-on job after topology cleanup and release/non-release terminal paths.
...
- **FR-009**: The release job MUST promote the exact validated **artifact set** — controller digest (`az acr import`) AND CNI digest (`oras copy`) — with no rebuild, prepare and verify the SAME semantic-version tag on both direct repositories, handle requested moving tags, and publish a canonical signed/attested release-index OCI tag containing both exact digests/version only after those checks pass.
- **FR-010**: The public ACR controller, CNI, and release-index repositories MUST be anonymously readable so external consumers can resolve the canonical index and pull both direct artifacts without credentials.
...
- **FR-012**: The workflow MUST expose typed `workflow_dispatch` inputs (`regions`, `validation_topologies`, `run_full_validation`, `release`, `release_version`, `moving_tags`, `keep_resources_on_failure`, `primary_subscription_id`, `secondary_subscription_id`) and implement Section 9.1. Release and `run_full_validation` force `validation_topologies=ss,xs`; moving tags are validated per requested version and passed through run metadata; retention is allowed only after failed non-release provisioning/validation, never after success, and is forbidden for releases.
...
- **FR-017**: A scheduled reaper workflow MUST authenticate independently to both configured subscriptions and delete expired run-tagged resource groups and dangling run-scoped role assignments in each, plus old run-correlated staging candidate tags and ACR pull tokens.
...
- **FR-019**: The pipeline MUST publish the CNI artifact by digest to staging, generate an SBOM, and distribute the exact validated bytes to each centraluseuap worker through ordered, bounded Run Command chunks. No command may exceed the configured safe payload; reassembly MUST verify expected size and SHA-256 before install and fail closed on missing/out-of-order/tampered chunks.
...
- **FR-024**: The pipeline MUST ensure the released artifact set is byte-identical to the same-run validated set and MUST NOT release either artifact independently. A release exists only when the canonical semantic-version release-index tag records both verified digests; partial direct-tag visibility is not completion.
...
- **NFR-011 (Artifact-set atomicity)**: The controller and CNI MUST be validated and released as one set. Observable atomicity is provided by a canonical signed release-index `${SEMVER}` tag published last; before that commit, failure removes prepared tags/restores moving tags. The manifest and index MUST record both exact digests.
...
| `workflow_dispatch` (manual) | Yes | Yes | Selected topology for non-release; forced `ss,xs` when `release=true` or `run_full_validation=true` | Yes for every selected topology; retention only on requested failed non-release validation/provisioning | If `release=true` AND all gates pass |
...
- Pull requests to `main` have no restrictive path filter, so controller/CNI source, modules, Dockerfile, Makefile, config, E2E scripts, and workflow changes all trigger checks.
...
- **Release rollback**: Before release-index commit, any failure removes prepared direct/index tags and restores moving tags, so no completed release exists. After commit, semantic tags and the canonical index are immutable; rollback means advancing BOTH direct moving tags to a previously released good index/version. Published immutable versions/indexes are not deleted.
...
| RD-021 | Use a signed/attested OCI release index, published under `${SEMVER}` only after both direct artifacts and moving-tag operations verify, as the sole atomic public commit point | Sequential direct-tag writes cannot be observably atomic; a final immutable index gives consumers/audits one completion marker while preserving required direct artifact repositories |
...
| ITEM-030 | Implement `install-cni-tt.sh` (generalizing the doc's `tt_install.sh`): pull by digest, transfer via bounded ordered Run Command chunks, checksum-verify reassembly, back up/install/swap/assert/restart on every worker, and leave the control-plane stock. Validation: TEST-012 covers a >one-command artifact, payload bounds, ordering/reassembly/checksum failure, all workers, and backup/control-plane behavior. | Done | scripts/e2e/install-cni-tt.sh, docs/transparent-tunnel-same-node-enforcement-test.md |
...
| ITEM-035 | Extend `promote-release.sh` and the release gate to promote both digests under one `${SEMVER}` without rebuild, require `validate_tt`, and publish the signed OCI release index last as the atomic completion marker. Validation: AC-023; partial failures never commit the index and prepared tags are removed. | Done | scripts/e2e/promote-release.sh, .github/workflows/e2e-validation-release.yml |
...
- 2026-08-21 — v1.3 — Clarified observable artifact-set atomicity through a signed/attested canonical OCI release index (RD-021), bounded ordered Run Command transfer for production-sized CNI binaries, final always-on staging candidate/token cleanup plus reaping, comprehensive PR triggering, and the full FR-012 dispatch/retention semantics. All completed epic/item statuses remain Done.
